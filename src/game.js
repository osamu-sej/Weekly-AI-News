...
const STAGES = [
  { name: 'STAGE 1: 月光残骸帯', duration: 30, tint: '#4efcff', accent: '#bdefff', bossName: '月蝕戦艦 SELENE', bossType: 'carrier', enemyBias: ['drone', 'drone', 'seeker'] },
  { name: 'STAGE 2: 紫電アステロイド', duration: 36, tint: '#ff4fd8', accent: '#9dff6a', bossName: '強襲蟷螂 VESPER', bossType: 'mantis', enemyBias: ['drone', 'seeker', 'blade', 'heavy'] },
  { name: 'STAGE 3: ネオン要塞中枢', duration: 42, tint: '#ffd166', accent: '#ff4fd8', bossName: '要塞核 ARK-OMEGA', bossType: 'fortress', enemyBias: ['seeker', 'blade', 'heavy', 'turret'] },
];
...
const POWERUP_TYPES = {
  ...
  bomb: { label: 'NOVA BOMB STOCK', color: '#7f8cff' },
};
...
function choosePowerupType(forcedOption = false) {
  ...
  const table = ['spread', 'missile', 'laser', 'shield', 'bomb'];
...
function collectPowerup(type) {
  ...
  if (type === 'bomb') {
    p.bombs = clamp(p.bombs + 1, 0, 4);
    addScore(700, `NOVA BOMB +${p.bombs}`);
    return;
  }
...
function spawnBoss() {
  ...
  const stageMeta = currentStage();
  const hp = 720 + stage * 420;
  state.enemies.push({
    x: W + 140,
    y: H / 2,
    baseY: H / 2,
    ...
    t: 0,
    type: 'boss',
    bossType: stageMeta.bossType,
    name: stageMeta.bossName,
  ...
  state.shake = 14;
  ui.status.textContent = `WARNING: ${stageMeta.bossName} 接近`;
}
...
function useNovaBomb() {
  ...
  state.shake = 24;
  state.enemyBullets = [];
  state.shockwaves.push({ x: p.x, y: p.y, r: 20, life: .72, maxLife: .72, color: '#7f8cff' });
  state.enemies.forEach(enemy => {
    enemy.hp -= enemy.type === 'boss' ? 180 : 999;
    burst(enemy.x, enemy.y, '#7f8cff', enemy.type === 'boss' ? 18 : 32, 1.35);
  });
  addScore(500, 'NOVA BOMB — 画面制圧');
}
...
function update(dt) {
  ...
  fireMissiles();
  if ((keys.has('e') || keys.has('b')) && bombArmed) {
    useNovaBomb();
    bombArmed = false;
  }
  if (!keys.has('e') && !keys.has('b')) bombArmed = true;
...
function updateEntities(dt) {
  ...
  state.shockwaves.forEach(wave => { wave.r += 980 * dt; wave.life -= dt; });
...
function fireBossPattern(enemy) {
  ...
  if (enemy.bossType === 'fortress' || (enemy.bossType === 'mantis' && enemy.phase > 0)) {
    for (let i = 0; i < 10; i += 1) {
      const a = (i / 10) * Math.PI * 2 + state.time * .3;
      const speed = enemy.bossType === 'fortress' ? 150 : 125;
      state.enemyBullets.push({ x: enemy.x - 18, y: enemy.y, vx: Math.cos(a) * speed, vy: Math.sin(a) * speed, r: 4.5, life: 4, color: enemy.color });
    }
  }
}
...
function updateUi() {
  ...
  ui.options.textContent = `${p.options.length}/4 S${p.spreadLevel} M${p.missileLevel} B${p.bombs}`;
...
function drawBackground() {
  ...
  drawStageSetpiece(stage);
}

function drawStageSetpiece(stage) {
  ctx.save();
  if (state.stage === 1) {
    ctx.globalAlpha = .28;
    const moon = ctx.createRadialGradient(W * .78, H * .28, 20, W * .78, H * .28, 190);
    moon.addColorStop(0, '#ffffff');
    moon.addColorStop(.32, stage.tint);
    moon.addColorStop(1, 'rgba(78,252,255,0)');
    ctx.fillStyle = moon;
    ctx.beginPath(); ctx.arc(W * .78, H * .28, 190, 0, Math.PI * 2); ctx.fill();
  } else if (state.stage === 2) {
    ctx.globalAlpha = .55;
    for (let i = 0; i < 9; i += 1) {
      const x = (W - ((state.time * (95 + i * 7)) % (W + 180))) + i * 90;
      const y = 95 + ((i * 83) % 520);
      ctx.strokeStyle = i % 2 ? stage.tint : stage.accent;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(x, y - 22); ctx.lineTo(x + 34, y - 5); ctx.lineTo(x + 20, y + 28); ctx.lineTo(x - 26, y + 19); ctx.closePath();
      ctx.stroke();
    }
  } else {
    ctx.globalAlpha = .24;
    ...
    ctx.lineWidth = 3;
    for (let i = 0; i < 5; i += 1) {
      const x = W - 260 + i * 80 - ((state.time * 25) % 80);
      ctx.strokeRect(x, 120 + i * 38, 180, 360 - i * 36);
    }
  }
  ctx.restore();
}
...
function drawBoss(enemy, hp) {
  drawGlow(enemy.x, enemy.y, enemy.r * 2.25, enemy.color || '#ffd166', .24);
  ctx.save();
  ctx.translate(enemy.x, enemy.y);
  ctx.rotate(Math.PI);
  const r = enemy.r;
  const g = ctx.createLinearGradient(-r, -r, r, r);
  g.addColorStop(0, enemy.bossType === 'carrier' ? '#103d5f' : '#3b1458');
  g.addColorStop(.52, enemy.bossType === 'mantis' ? '#244b22' : '#a6474f');
  g.addColorStop(1, enemy.color || '#ffd166');
  ctx.fillStyle = g;
  ctx.strokeStyle = '#ffffff';
  ctx.lineWidth = 4;
  ctx.beginPath();
  if (enemy.bossType === 'mantis') {
    ctx.moveTo(r * 1.34, 0); ctx.lineTo(r * .42, -r * .32); ctx.lineTo(-r * .18, -r * 1.12); ctx.lineTo(-r * .44, -r * .34); ctx.lineTo(-r * 1.22, -r * .78); ctx.lineTo(-r * .7, 0); ctx.lineTo(-r * 1.22, r * .78); ctx.lineTo(-r * .44, r * .34); ctx.lineTo(-r * .18, r * 1.12); ctx.lineTo(r * .42, r * .32);
  } else if (enemy.bossType === 'fortress') {
    ctx.moveTo(r * 1.1, 0); ctx.lineTo(r * .58, -r * .88); ctx.lineTo(-r * .72, -r * .72); ctx.lineTo(-r * 1.05, 0); ctx.lineTo(-r * .72, r * .72); ctx.lineTo(r * .58, r * .88);
  } else {
    ctx.moveTo(r * 1.38, 0); ctx.lineTo(r * .62, -r * .55); ctx.lineTo(-r * .58, -r * .82); ctx.lineTo(-r * 1.08, -r * .22); ctx.lineTo(-r * .72, 0); ctx.lineTo(-r * 1.08, r * .22); ctx.lineTo(-r * .58, r * .82); ctx.lineTo(r * .62, r * .55);
  }
  ctx.closePath();
  ctx.fill(); ctx.stroke();
  ...
  ctx.fillStyle = hp > .45 ? (enemy.color || '#ffd166') : '#ff4fd8';
  ctx.fillRect(W * .25, 74, W * .5 * hp, 8);
  ctx.fillStyle = '#ffffff';
  ctx.font = '900 15px "Exo 2", sans-serif';
  ctx.textAlign = 'center';
  ctx.fillText(enemy.name, W * .5, 66);
}
...
function drawShockwaves() {
  state.shockwaves.forEach(wave => {
    ...
    ctx.beginPath(); ctx.arc(wave.x, wave.y, wave.r * .55, 0, Math.PI * 2); ctx.stroke();
    ctx.restore();
  });
}
